...
public class Thumbnails : MonoBehaviour
{
    ...
    [Header("Change the Aspect Ratio for each Thumbnail")]
    [Tooltip("A thumbnail will be created with this base name in your project root")]
    public string assetBaseName = "Asset";
    ...
    IEnumerator CaptureFrame()
    {
        ...
        if (videoFrame < framesToCapture)
        {
            ...
            SavePng();
            videoFrame++;
            Debug.Log("Rendered image " + videoFrame);
            videoFrame++;
        }
        else
        {
    ...
    void SavePng()
    {
        string name = string.Format("{0}_{1}.png", folderName, screenHeight);
